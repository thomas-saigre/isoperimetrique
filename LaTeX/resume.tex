...
\begin{itemize}
    \item \emph{problème isopérimétrique} : \[\max_{D \in \C} \A(D) \text{ avec } \C = \{ D \in K, \p(D) = p_0 \} \label{ida}\tag{\IDA}\]
    \item \emph{problème iso-aire} : \[\min_{D \in \C} \p(D) \text{ avec } \C = \{ D \in K, \A(D)=c \} \label{idp}\tag{\IDP}\]
\end{itemize}
...
Nous voulons résoudre le problème iso-périmétrique \ref{ia} avec $p_0=2\pi$ avec comme contrainte que les domaines considérés soit à bord $\C^1$, ce problème se réécrit :
\[\max_{z \in \mathcal{E}} \A(z) \label{eq:p}\tag{$\P$}\]


Avec $\mathcal{E} = \{ z \in C^1([-\pi,\pi], \R), z(-\pi)=z(\pi), |z'(s)|=1 \}$.

Et $\A(z) = \frac{1}{2} \Im\left(\int_{-\pi}^{\pi}{\bar z(t) z'(t) dt}\right) = \frac{1}{2} \int_{-\pi}^{\pi}{x\d y-y\d x}$.

%\includegraphics[]{}

Pour $z\in \mathcal{C}$, nous notons $x$,$y : [-\pi,\pi] \rightarrow \R$ respectivement les parties réelles et imaginaires de $z$.

En effet :
\[ \p(z) = \int_{-\pi}^{\pi}{|z'(t)|dt} = 2\pi = p_0 \label{eq:e}\tag{\textsf{I}}\]
\[ \A(z) = \A(\Gamma_z) \text{ avec $\Gamma_z$ l'ensemble délimité par le graphe de $z$} \label{eq:f}\tag{\textsf{II}}\]
...
&\underset{\ref{eq:g}}{\Longleftrightarrow}& \max_{z \in \mathcal{E}'} \A(\Gamma_z) \text{ avec } \mathcal{E}' = \{ z \in \C^1([-\pi,\pi]), \p(z) = 2\pi \} \\
&\underset{\ref{eq:e}+\ref{eq:f}}{\Longleftrightarrow}& \max_{z \in \mathcal{E}} \A(z) \text{ avec } \mathcal{E} = \{ z \in \C^1,  z(-\pi)=z(\pi), |z'(s)|=1 \}
\end{eqnarray*}
...
Pour un $z \in \mathcal{E}$ tel que $z=\sum_{n\in \Z}{c_ne_n}$, Nous voulons prouver que $z = c_0 + c_1 e_1$ soit un cercle. L'idée est de regarder la déviation $w(s) = z(s) - (c_0+c_1e^{is}) = \sum_{n\in \Z\backslash \{0,1\}}{c_ne_n}$ et de prouver que $\Vert w\Vert_{H^1} = 0$ \footnote{$H^1 = \{ w \in L^2(\R), \exists g \text{ dérivé au sens des distributions} \}$ et $\Vert w \Vert_{H^1} = \Vert w \Vert_{L^2} + \Vert g \Vert_{L^2}$} lorsque $A=\pi$.
...
D'autre part :
\[ \Vert w\Vert_{H^1} \leq \frac{5}{2} \left(1 - \frac{A}{\pi}\right) \label{in:s} \tag{\textsf{S}} \]
...
\[\max_{D \in \C} \A(D) \]
Avec $\mathcal{C} = \{ D \text{ un convexe borné à bords $C^1$ de } \R^2, \p(D) = 2\pi \}$

\`A pour solution un cercle.
\end{coro}
...
&\simeq& \sum_{i=-1}^{n-1}{\sqrt{\vphantom{f_n^2}{(f_{i+1}-f_i)}^2+{(x_{i+1}-x_i)}^2}}\\
&\simeq& \sqrt{f_0^2+h^2} + \sum_{i=0}^{n-1}{\sqrt{\vphantom{f_n^2}{(f_{i+1}-f_i)}^2+h^2}} + \sqrt{f_{n-1}^2+h^2} 
\end{eqnarray*}

\begin{rem}
En discrétisant l'intégrale $\int_{x=-\ell}^{\ell}{\sqrt{x^2+f(x)^2}}$ par la formule des rectangles , nous retombons sur l'expression ci-dessus.
\end{rem}
