...
\only<2>{%
    ...
    Le but est de prouver que si $Aire(z)$ est maximale alors $z = c_0 + c_1 e_1$ (correspondant à un cercle car $e_1(s)=e^{is}=\cos s+i\sin s$ dans le plan complexe, cela décrit un cercle).    
}
...
\only<3>{%
    ...
    \ss
    \textbf{Condition d'optimalité} : $Aire(z) \leqslant \pi$, et le cercle $z = c_0 + c_1 e_1$ vérifie cette condition.    
}
\only<4>{%
    On introduit la \emph{déviation} $w=z-(c_0+c_1e_1)$, qui est la distance de l'arc au cercle, et on montre, avec la majoration $1+n^2\leqslant \frac{5}{2}(n^2-n)$ pour $n\neq 0,1$, que :
    \[ \Vert w\Vert_{H^1}^2 \leq \frac{5}{2} \left(1-\frac{Aire(z)}{\pi}\right) \]
